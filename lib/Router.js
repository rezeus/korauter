...
function register(methods, name, /** @type {string} */ path, handler, meta, namePrefix = '', pathPrefix = '', defaultMeta = {}) {
  ...
  if (name !== '') {
    ...
    const regexpedPath = pathToRegexp.compile(prefixedPath);
    this.namedRoutes[prefixedName] = regexpedPath;
  }
...
/**
 * Resolves the URL if it wasn't so far and then
...
async function handle(ctx, next) {
  ...
  if (!handler) {
    ...
    return next(); // FIXME continue or `CTX.THROW(404)`?
  }
...
function Router(options) {
  ...
  this.trees = {};
  this.namedRoutes = {}; // name -> pathToRegexp(URL)
...
Router.prototype.url = function url(name, params) {
  const regexpedPath = this.namedRoutes[name];

  if (regexpedPath) {
    return regexpedPath(params);
  }

  throw new Error(`Named URL (${name}) does not exist.`);
};
...
Router.prototype.head = function (...args) { this.defaultLayer.head.call(this.defaultLayer, ...args); };
Router.prototype.get = function (...args) { this.defaultLayer.get.call(this.defaultLayer, ...args); };
Router.prototype.post = function (...args) { this.defaultLayer.post.call(this.defaultLayer, ...args); };
Router.prototype.put = function (...args) { this.defaultLayer.put.call(this.defaultLayer, ...args); };
Router.prototype.del = function (...args) { this.defaultLayer.del.call(this.defaultLayer, ...args); };
Router.prototype.patch = function (...args) { this.defaultLayer.patch.call(this.defaultLayer, ...args); };
// TODO Consider 'options', 'trace', 'connect'

Router.prototype.scope = function (...args) { this.defaultLayer.scope.call(this.defaultLayer, ...args); };
